...
type Model<T extends ModelData = Record<string, unknown>> = Omit<ModelImplementation<T>, ModelEventMethod> & {
    on<Name extends ModelEventName<T>>(eventName: Name, listener: (...arguments_: ModelEventArguments<T, Name>) => void): Model<T>;
    once<Name extends ModelEventName<T>>(eventName: Name, listener: (...arguments_: ModelEventArguments<T, Name>) => void): Model<T>;
    addListener<Name extends ModelEventName<T>>(eventName: Name, listener: (...arguments_: ModelEventArguments<T, Name>) => void): Model<T>;
    off<Name extends ModelEventName<T>>(eventName: Name, listener: (...arguments_: ModelEventArguments<T, Name>) => void): Model<T>;
    removeListener<Name extends ModelEventName<T>>(eventName: Name, listener: (...arguments_: ModelEventArguments<T, Name>) => void): Model<T>;
    emit<Name extends ModelEventName<T>>(eventName: Name, ...arguments_: ModelEventArguments<T, Name>): boolean;
};
interface ModelConstructor {
    new <T extends ModelData = Record<string, unknown>>(modelData?: T, validator?: (data: unknown) => boolean): Model<T>;
}
